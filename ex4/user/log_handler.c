...
const char *log_format = "%-19s  %-15s  %-15s  %-8s  %-8s  %-8s  %-6s  %-25s %u\n";
...
void log_row2str(const log_row_t *log_row, char *str)
{
    ...
    sprintf(str, log_format, timestamp, src_ip, dst_ip, src_port, dst_port, protocol, action, reason, log_row->count);
}
