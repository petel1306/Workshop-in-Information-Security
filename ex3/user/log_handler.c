...
void log_row2str(const log_row_t *log_row, char *str)
{
    ...
    char timestamp[30], src_ip[30], dst_ip[30], src_port[8], dst_port[8], *protocol, *action, reason[30];

    time2str(timestamp, log_row->timestamp);
    ip2str(src_ip, log_row->src_ip);
    ip2str(dst_ip, log_row->dst_ip);
    port2str(src_port, log_row->src_port);
    port2str(dst_port, log_row->dst_port);
    ...
    sprintf(str, "%-19s  %-15s  %-15s  %-8s  %-8s  %-8s  %-6s  %-24s %u\n", timestamp, src_ip, dst_ip, src_port,
            dst_port, protocol, action, reason, log_row->count);
}

void log_headline(char *str) {
    sprintf(str, "%-19s  %-15s  %-15s  %-8s  %-8s  %-8s  %-6s  %-24s %s\n", "timestamp", "src_ip", "dst_ip", "src_port",
            "dst_port", "protocol", "action", "reason", "count");
}
